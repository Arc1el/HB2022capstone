...
if (typeof navigator.mediaDevices.getDisplayMedia === 'undefined') {
  ...
  function recorder(stream) { // stream: 상대방 video
  ...
  mediaRecorder.addEventListener('stop', function () {
    ...
    xhr.open("POST", "https://arc1el.iptime.org:8000/streamdata", false);
    xhr.send(formdata);
    
  });

  btn_rec = document.getElementById("btn_rec");
  btn_stop = document.getElementById("btn_stop");

  btn_rec.onclick = () => { mediaRecorder.start(); };
  btn_stop.onclick = () => { mediaRecorder.stop(); };
}
  ...
  transport.on('connectionstatechange', async (state) => {
    switch (state) {
      ...
      case 'connected':
        ...
        recorder(str);
        document.querySelector('#remote_video').srcObject = str;
